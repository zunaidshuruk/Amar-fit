...
7. [ ] **Guided timed workouts** — structured JSON workout generation (exercise/duration/rest, replacing freeform text), interval timer with auto-advance, in-app embedded YouTube video per exercise, MET-based calorie calculation — feeds real session timing into #6's Exercise write
   - *(Partial — 7a done: `WorkoutPlan`/`WorkoutExercise` JSON schema, `generateStructuredWorkout()` via Gemini `responseMimeType=application/json`, Room v25 migration adding `SavedWorkout.structuredJson`, and structured exercise-list rendering in `WorkoutScreen`/`SavedWorkoutDetailView` (falls back to legacy `content` text for old saved workouts). 7b — interval timer, in-app YouTube embed, MET-based live calorie calc — not started. Note: legacy `generateAIWorkout()`/`workoutPlan` StateFlow are now dead code in the UI, kept intentionally per scope, safe to remove once 7b lands.)*
