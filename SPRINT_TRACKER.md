...
3. [x] **Today tab redesign** — 3-page swipeable stat carousel, action row (Log/Start/Edit), chronological activity timeline feed, tile customization ("Edit Focus" with 7 curated metrics) *(Updated: top carousel replaced with fixed Google-Health-style layout featuring Daily Steps circular ring gauge + 3-row pill stack and 3x2 pill grid; carousel customization removed; rebuilt Edit Focus as full-screen picker with unbounded large and small metric tiles, live data previews, and backward-compatible slot storage)*
